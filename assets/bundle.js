...
/* Metro Responsive Script */
$z( window ).on('load', function() {
	sizeContent();
	clickMenu();
	setTimeout(sizeContent, 500); // needed in case a slider or carousel widget is displayed
});	
$z(window).resize(sizeContent);
$z(document).ready(mobileNav);

function sizeContent() {		
	var startWidth = $z(window).width();
	if (startWidth > 984) {
		$z("#left").height($z(document).height());
		$z("#right").height($z(document).height());
		if ($z(document).height()-96 > $z("#sidebar").height()) {
			$z("#sidebar").height($z(document).height()-96);
		}
		if ($z(document).height()-96 > $z("#content").height()) {
			$z("#content").height($z(document).height()-96);
		}
...
function clickMenu() {
	var thisLeftHeight = $z("#left").height();
	var thisRightHeight = $z("#right").height();
	var thisLogoHeight = $z("#logo").height();
	var thisNavigationHeight = $z("#navigation").height();
	var thisContentHeight = $z("#content").height();
	var thisSidebarHeight = $z("#sidebar").height();
	var thisRefHeight = thisLogoHeight+thisNavigationHeight+48;


	$(document).on('click', function(e) {
		if (!$(e.target).closest('#navigation ul li').length) {
			$z("ul.submenu").hide();
			$z("#navigation ul li a").removeClass("selected");
			$z("#left").height(thisLeftHeight);
			$z("#right").height(thisRightHeight);
		}
	});

	$z("#navigation ul li").click(function() {
		$z.fx.off = true;
		var thisSubmenu = $z(this).find("ul.submenu");
		$z("ul.submenu").not(thisSubmenu).hide();
		$z("ul.submenu").not(thisSubmenu).parent("#navigation ul li").find("a:first").removeClass("selected");
		$z(this).find("ul.submenu").toggle(function() {
			$z(this).parent("#navigation ul li").find("a:first").toggleClass("selected");
			var thisSubmenuHeight1 = $z(this).height()+168;
			var thisSubmenuHeight2 = $z(this).height()+72;
			if ((thisSubmenuHeight1) > thisContentHeight) {
				$z("#left").height(thisSubmenuHeight1+96);
				$z("#right").height(thisSubmenuHeight1+96);
				$z("#content").height(thisSubmenuHeight1);
				$z("#sidebar").height(thisSubmenuHeight1);
			} else if (thisSubmenuHeight2 < thisRefHeight) {
				$z(this).height(thisRefHeight);
			}
		});
	});
}
...
function mobileNav() {
	$z("#mobile-menu").click(function() {
		$z.fx.off = true;
		$z("#mobile-navigation").toggle(function() {
			$z("#mobile-menu").toggleClass("selected");
			$z("#left").toggleClass("selected");
			$z("#right").toggleClass("selected");
			$z("#xmenu").toggleClass("selected");
			$z("#sidebar").toggleClass("selected");
			$z("#content").toggleClass("selected");
			$z("#footerbox").toggleClass("selected");
		});
	});
	$z("#mobile-more").click(function() {
		$z(".news").slideToggle(function() {
